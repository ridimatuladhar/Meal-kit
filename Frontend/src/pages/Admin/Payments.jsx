...
const Payments = () => {
  ...
  const [paymentStatusFilter, setPaymentStatusFilter] = useState(''); // Separate payment status filter
  const [sortOption, setSortOption] = useState('payment_latest'); // Keep for actual sorting
  ...
  useEffect(() => {
    fetchOrders();
  }, [filter, paymentStatusFilter, sortOption, currentPage]); // Added paymentStatusFilter to dependencies

  const fetchOrders = async () => {
    try {
      setLoading(true);
      // Fixed filter parameter - use empty string for 'all'
      const filterParam = filter === 'all' ? '' : filter;
      
      const response = await fetch(
        `http://localhost:3000/api/order/admin/all?status=${filterParam}&sort=${sortOption}&page=${currentPage}&limit=${ordersPerPage}`,
        {
          method: 'GET',
          headers: {
  ...
  const updatePaymentStatus = async (orderId, newPaymentStatus) => {
    try {
      const response = await fetch(
        `http://localhost:3000/api/order/admin/${orderId}/payment-status`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            ...
            note: `Payment status updated to ${newPaymentStatus} by admin`,
          }),
        }
      );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex-1 ml-40 p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600 border-b border-gray-200 pb-4">View and manage all payments</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
            <div>
              ...
              <select
                ...
                className="w-full rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black text-sm py-2 px-2"
              >
                <option value="payment_latest">Latest Payment</option>
                <option value="payment_oldest">Oldest Payment</option>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Payment Status</th>
                  </tr>
                </thead>
                      ...
                      <td className="px-6 py-2">
                        ...
                        {order.paymentMethod === 'cod' ? (
                          <select
                            ...
                            className="mt-1 block w-full px-2 py-1 text-xs rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black"
                          >
                            <option value="pending">Pending</option>
